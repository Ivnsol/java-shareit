...
public interface BookingStorageForList extends JpaRepository<AllBookingsAsList, Long> {
    ...
    List<AllBookingsAsList> findAllByBookerOrderByEndDesc(@Param("owner") Long owner);

    @Query("SELECT b FROM AllBookingsAsList b WHERE b.booker.id = :booker AND b.status = :status ORDER BY b.start DESC")
    List<AllBookingsAsList> findBookingByBookerAndStatusOrderByEndDesc(@Param("booker") Long booker, @Param("status") StatusOfBooking status);
