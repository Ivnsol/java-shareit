...
public interface BookingStorageForList extends JpaRepository<AllBookingsAsList, Long> {
    @Query("SELECT b FROM AllBookingsAsList b WHERE b.booker.id = :owner")
    List<AllBookingsAsList> findAllByBookerOrderByEndDesc(@Param("owner") Long owner, Pageable pageRequest);

    @Query("SELECT b FROM AllBookingsAsList b WHERE b.booker.id = :booker AND b.status = :status")
    ...
    @Query("SELECT b FROM AllBookingsAsList b WHERE b.item.id IN :itemsId AND b.status = :status")
